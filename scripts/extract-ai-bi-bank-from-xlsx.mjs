#!/usr/bin/env node
/**
 * Extract the published Senior Databricks AI/BI & Genie Consultant question
 * bank from the source spreadsheet ("AI BI G Question bank 1.1.xlsx").
 *
 * Sibling of scripts/extract-question-bank-from-xlsx.mjs (RSA bank): the same
 ...
 *     lives in `ai_bi_genie_assessment_question`;
 ...
 *
 * Usage:
 *   node scripts/extract-ai-bi-bank-from-xlsx.mjs [source.xlsx|source.csv] [bank.json]
 *
...
const source = process.argv[2] || 'AI BI G Question bank 1.1.xlsx';
const outPath = process.argv[3] || 'data/ai-bi-bank.json';
...
const records = parsed.rows.map((row) => {
  ...
  const { prompt, options } = splitEmbeddedObjective(row.ai_bi_genie_assessment_question);
